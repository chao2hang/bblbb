...
昵称颜色 Token 使用固定白名单：`blue`、`purple`、`green`、`gold`、`red`、`teal`、`pink`、`rainbow`、`breathing`、`gradient_sunset`、`gradient_ocean`、`gradient_aurora`；渐变、彩虹与呼吸光由客户端固定样式渲染，管理员不能提交 CSS。在此之上提供**可配置装扮样式库**（M07-SHOP-UI-10，`cosmetic_defs` 表）：管理员自定义命名样式（昵称颜色：纯色/渐变 2–5 色标/呼吸光；头像框：颜色/线宽/光晕/形状/脉冲），`style_json` 由服务端 schema 校验（颜色 `#rrggbb`、动画枚举、时长钳制 800–30000ms），商品 Token 以 `nickname.color.<id>` / `avatar.frame.<id>` 引用定义，公开投影附带名称与结构化样式；归档为软删，已引用商品降级为默认样式。头像框另支持经附件安全扫描的图片资源（PNG 或 APNG 动图，即 Steam 动效头像框同款方案：安全管线只剥离元数据块、不重编码，动画原样保留并在前端 `<img>` 循环播放）；管理员在商城商品表单中通过两阶段上传后保存附件 UUID；公开用户投影只返回 ready/public 资源 UUID，前端通过稳定内容端点读取。
...
- `reaction_pack`、`utility`、`title_prefix` 不装备到展示槽位：商品 `slot` 允许缺省（存空串），不参与 equip 查询；装备类商品仍必须携带合法槽位。
- 装备不扣费；购买后可以随时卸下或换装。
...
后台可以配置：商品名称、说明、图标、Token、价格、货币、库存、销售时间、等级门槛、用户限购、有效期、展示槽位、安全审核状态、是否可退款和活动标签。新建/编辑表单按商品类型展示独立设置界面（昵称色板与特效选择、头像框内置样式网格或 PNG/APNG 上传、徽章多选、装饰单选），槽位与 Token 由界面按选择自动生成；「高级」折叠区保留原始 Token 编辑，仍受后端注册白名单校验。上传资源必须经过附件安全处理；商品图标优先使用内置图标或受控附件，禁止远程 URL。
