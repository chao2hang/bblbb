...
- 提供关注、收藏和受参与者隔离保护的双人私信；私信为普通请求/响应接口，不承诺实时通信。
...
- 实时聊天和实时音视频通信；但允许通过受控视频插件引用第三方视频页面/流媒体。v1.0 的双人私信不属于实时通信能力，已纳入产品范围。
...
- S3 凭证只由后端读取，支持预签名上传/下载但不能绕过服务端 HEAD、hash、媒体类型/图片处理和权限校验。
