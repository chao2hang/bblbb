...
> 1. Steam 动效头像框、Steam 资料背景及彩色昵称上架弹窗全面开放商品属性配置；
> 2. 支持自定义有效期限（永久有效、7天体验、30天月卡、90天季卡、365天年卡及自定义天数）；
> 3. 支持配置上架库存数量（限量发售或留空不限数量）；
> 4. 支持设定每人限购数量与购买等级门槛；
> 5. 表单采用两列自适应 Grid 布局，兼具高效选品与精准定价调控。
> 发布顺序：frontend。

### 商城上架商品属性配置增强（M07-SHOP-STUDIO / M18-ADMIN-OPS）

- **多维度销售属性配置**：
  - `SteamFramePricingModal` / `SteamBackgroundPricingModal` / `QuickNicknameModal` 新增：
    - **有效期限**：提供常用预设（永久 / 7天 / 30天 / 90天 / 365天）以及自定义天数，自动计算 `validity_seconds` 秒数；
    - **上架库存**：支持设定 `stock_remaining` 限量发售，留空默认为不限库存；
    - **限购控制**：支持设定 `quantity_limit`，防止恶意扫货；
    - **等级门槛**：支持设定 `required_level`（TL0–TL4），灵活作为高阶用户专享福利。
- **表单布局与交互优化**：
  - 选品定价抽屉采用响应式双列栅格排布，输入框自带明确占位提示与辅助说明，保持简洁高效。
- **测试覆盖**：
  - `admin-shop-view.test.ts` 新增上架弹窗字段存在性与配置渲染断言；前端全部 156 测试文件、1099 测试用例通过。

## v1.0.0-rc.10 — 2026-09-21（后台商城管理重构：主业务 Tab 分区彻底消除列表与装扮样式库冲突）

> 交互体验优化版本。彻底解决管理员在 `/admin/shop` 页面面对双重列表平铺的逻辑冲突与操作痛点：
> 1. 顶层分设「商品货架」、「装扮样式库」与「订单记录」三大主业务 Tab；
> 2. 默认聚焦商品货架，杜绝“已生效装扮”在上半屏遮挡压迫商品列表；
> 3. 统一全局分类 Tag 过滤与搜索条，计数与筛选状态与当前激活的分区完美同步；
> 4. 保持无 JS 与 SSR 渲染无缝兼容，支持 `?tab=products|cosmetics|orders` 链接直达。
> 发布顺序：frontend。

### 后台商城管理体验重构（M07-SHOP-UI / M18-ADMIN-OPS）

- **主业务分区 Tab 解耦**：
  - 建立【商品货架】（在售商品、定价、库存、批量上下架）、【装扮样式库】（视觉 Token 模版定义、归档/恢复、工坊引导）与【订单记录】（购买流水、补偿退款）三重视图分离体系。
  - 默认展示【商品货架】，一屏直达商品管理，解决“一个列表 又一个已生效装扮 逻辑冲突”的页面割裂问题。
- **状态与工具栏收敛**：
  - 分类 Tag（全部 / 头像框 / 主页背景 / 彩色昵称 / 其他）动态按当前激活 Tab 统计数量，杜绝两份数据串台与误导。
  - 搜索框与卡片/列表视图切换收敛为唯一工具条，精准控制当前视图。
- **SSR 兼容与测试对齐**：
  - 支持服务端 `?tab=` 参数直接投影指定分区；
  - `admin-shop-view.test.ts` 新增主 Tab 切换与分区隔离断言；前端全量 156 个测试文件、1098 个测试用例 100% 通过。
