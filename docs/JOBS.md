...
集成测试覆盖 `backend/tests/crash_recovery.rs`，四种崩溃点：

| 崩溃点 | 恢复机制 | 结果 |
|---|---|---|
| 领取后（job lease 未释放） | lease 过期 → 其他 worker 重领（attempts+1） | 最终 succeeded，结果只一次 |
| 业务调用后、提交前（outbox 消费者事务） | 整事务回滚：去重标记+副作用+标记 sent 一起消失 | 重投重新执行，副作用恰好一次 |
| 提交后 / 投递系统重投 | `outbox_consumed` 去重标记持久 | 重复投递被跳过，不重复副作用 |
| job 处理器副作用提交后、complete 前 | 效果行唯一键幂等（处理器重跑命中唯一键即跳过） | 崩溃重跑不重复效果 |

原则：任何“至少一次”执行都必须搭配可验证的幂等机制（事务性收件箱、
唯一键效果行或目标侧幂等键）。
