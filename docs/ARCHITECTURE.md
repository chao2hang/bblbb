...
附件业务只依赖存储适配器接口，不直接依赖本地路径或 S3 SDK。`LocalStorage` 与 `S3Storage` 都必须实现临时对象、提交、读取、删除和存在性检查；预签名能力为可选优化，不能替代后端鉴权和内容处理。S3 凭证只存在 Rust 进程可读的受保护配置中，前端只能看到脱敏连接状态。

对象存储不可用时，上传保持 `pending` 或失败并可重试；帖子和回复事务不得引用未进入 `ready` 状态的附件。
