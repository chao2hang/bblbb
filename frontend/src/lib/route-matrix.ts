...
export const PROTOTYPE_ROUTE_MATRIX: PrototypeRouteEntry[] = [
  ...
  { prototype: '/search', production: '/', status: 'shipped', milestone: 'M3', note: '统一 Header 全局搜索，去除独立搜索页' },
  ...
  { prototype: '/admin/roles', production: '/admin/roles', status: 'shipped', milestone: 'M3', note: '角色目录 + 权限矩阵保存 + 创建角色（role.manage；If-Match + reason + recent-auth）' },
  { prototype: '(无原型页)', production: '/admin/assignments', status: 'shipped', milestone: 'M3', note: '角色委派（用户搜索 + 角色授予/撤销，role.manage + 审计 reason）' },
  ...
  { prototype: '/admin/tags', production: '/admin/tags', status: 'shipped', milestone: 'M3', note: '标签管理（创建/编辑/启停/合并，tag.manage；If-Match + reason）' },
  ...
  { prototype: '/admin/levels', production: '/admin/levels', status: 'shipped', milestone: 'M13', note: '等级管理（2026-09 合并单轨：LinuxDo 信任等级 TL0–TL4 + 按信任等级取档的附件配额）' },
