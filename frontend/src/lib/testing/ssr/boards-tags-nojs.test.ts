...
describe('M03-UI-06 板块详情 SSR', () => {
  it('详情 + 权限提示（members + readonly）在 SSR HTML 中', () => {
    ...
    const { body } = render(BoardDetailPage, {
      ...
      props: { data: { board, posts: [post], error: null, sort: 'latest', q: '', following: false, authed: true, tags: [] } }
    });
    expect(body).toContain('技术分享');
  ...
  it('无帖子 → 空状态可读', () => {
    const { body } = render(BoardDetailPage, {
      props: { data: { board: rootBoard, posts: [], error: null, sort: 'latest', q: '', following: false, authed: true, tags: [] } }
    });
