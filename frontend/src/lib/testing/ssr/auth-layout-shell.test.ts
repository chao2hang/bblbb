// 登录与注册页独立全屏布局 SSR 测试
// 断言：
// 1. /login 与 /register 路由根布局不渲染前台 Navbar 与 BottomNav；
// 2. /login 与 /register 渲染独立全屏认证视口（auth-viewport-main）与 app-shell--auth；
// 3. /login 与 /register 仍可正常输出主要内容（children）与 Toast 宿主。
...
describe('认证页独立全屏布局系统（无前台 Header 与 BottomNav）', () => {
  it('在 /login 路径下：根布局不渲染前台 Navbar 与 BottomNav', () => {
    (page as any).url.pathname = '/login';
  ...
  it('在 /register 路径下：根布局同样不渲染前台 Navbar 与 BottomNav，采用独立全屏认证视口', () => {
    (page as any).url.pathname = '/register';
    const registerSnippet = createRawSnippet(() => ({
      render: () => '<div id="register-test-content">注册测试表单</div>'
    }));
    const { body } = render(RootLayout, {
      props: {
