-- BBLBB sanctions (M05-SCHEMA-03, MySQL)
--
...
CREATE TABLE sanctions (
    ...
    -- 注：MySQL 8 禁止 CHECK 引用参与 ON DELETE SET NULL 的列（错误 3823）。
    -- 生产代码从不物理删除 users 行（匿名化就地更新，见 users::deletion），
    -- SET NULL 实际不会触发；RESTRICT 保持 CHECK 不变量且三库语义一致。
