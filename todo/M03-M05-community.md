...
- [ ] `M03-SCHEMA-06` `P0` `[30m]` 为 role、permission、board 和 assignment 的删除/停用语义设置外键与应用约束。
- [ ] `M03-SCHEMA-07` `P0` `[45m]` 编写三数据库 schema 等价 Fixture，覆盖唯一性、过期 assignment 和非法状态。
- [ ] `M03-SCHEMA-08` `P1` `[30m]` 同步 `docs/SCHEMA.md`、状态枚举、事件目录和迁移清单。

## M03-PROFILE：资料投影、隐私与注销匿名化

**元数据：** `P0` · `owner=unassigned/backend-users` · `risk=high` · `depends=M03-SCHEMA` · `blocked=none`
