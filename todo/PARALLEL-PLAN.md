...
| W6 (M5) | 进行中 | — | `aaae708`(M05-SCHEMA)、`c90e8db`(SCHEMA 仪表盘)、`ed909fe`(M05-RISK) | M05-SCHEMA 闭环（8 任务，agent A）；M05-RISK 闭环（9 任务，主代理实现——B 域 M07-LEDGER 子代理 4 次外部 API 失败后降级，见 agents.md）；M05-CASES/SANCTIONS/APPEALS/NOTIFY/UI 逐包推进 |
