...
| W15 (M15) | 已完成（含 4 项外部阻塞） | 2026-08-08 | M15 主提交（见下） | M15 收口：42 任务 = 38 `[x]` + 4 `[!]`（684/783 完成，7 阻塞）；后端观测模块（JSON 日志+脱敏、/metrics loopback-only、领域计数器）、`--worker` 模式与 SIGTERM 优雅停机实测（HTTP 0.30s/worker 0.04s）；deploy 全套（bundle/Caddy/systemd/release.sh/startup-checks/测试 PASS=26）；备份恢复真实演练 RPO=0/RTO=0.18s + verify.sh/attachments/OIDC keys；迁移升级演练 apply_ms=68/lock_events=0；冒烟 PASS=14；13 个 Runbook + 值班矩阵 + 非作者执行记录；alerts-drill PASS=71；日志脱敏扫描 CLEAN；clippy 0 / cargo test 143 binaries 0 fail / migration_equivalence 绿 / check-roadmap exit 0；4 项 `[!]`（生产主机部署、MySQL/MariaDB、S3、SMTP）复查日期 2026-09-07 |
