...
> 状态：历史执行计划（ARCHIVED）；以下 87 个工作包 / 783 个叶子任务和 Wave 记录截至 2026-08-08，仅保留作过程证据。当前进度、下一任务和验收标准以 [`../TODO.md`](../TODO.md) 为准。
...
| W15 (M15) | 已完成（含 4 项外部阻塞） | 2026-08-08 | `468883e` + commit 回填 | M15 收口：42 任务 = 38 `[x]` + 4 `[!]`（684/783 完成，7 阻塞）；后端观测模块（JSON 日志+脱敏、/metrics loopback-only、领域计数器）、`--worker` 模式与 SIGTERM 优雅停机实测（HTTP 0.30s/worker 0.04s）；deploy 全套（bundle/Caddy/systemd/release.sh/startup-checks/测试 PASS=26）；备份恢复真实演练 RPO=0/RTO=0.18s + verify.sh/attachments/OIDC keys；迁移升级演练 apply_ms=68/lock_events=0；冒烟 PASS=14；13 个 Runbook + 值班矩阵 + 非作者执行记录；alerts-drill PASS=71；日志脱敏扫描 CLEAN；clippy 0 / cargo test 144 binaries 0 fail / migration_equivalence 4 passed / check-roadmap exit 0；4 项 `[!]`（生产主机部署、MySQL/MariaDB、S3、SMTP）复查日期 2026-09-07 |
| W16 (M16) | 已完成（含 2 项外部阻塞） | 2026-08-08 | `d953004` + commit 回填 | M16 收口：48 任务 = 46 `[x]` + 2 `[!]`（730/783 完成，9 阻塞）；稳定错误码四方一致（docs/OpenAPI/backend/frontend 106 码，check-code-fixtures.rb 强制，领域错误转换输出稳定码）；状态机迁移矩阵 + check-state-machine-matrix.rb；上一版本 client 兼容（compat/frozen-client + check-client-compat.rb 193/193）；契约边缘测试 harness_contract；CI 四层（CI-LAYERS.md + nightly/release-rc.yml）；ASVS 基线 + 泄漏扫漏 + scan.sh（cargo audit 4 项处置/SBOM 634）；S3 mock 故障注入 adapter.rs（403/404/429/5xx/multipart/预签名）+ faults.rs 不变量 + economy step-injection 全回滚；性能实测（1M 帖/1137MB DB、p95 详情 17.6ms/SSR 19-24ms/列表 1207ms 登记、RSS 35MB）；release bundle bblbb-migrate 命名修复 PASS=26；Playwright 194 + smoke 14 + drills 全绿；2 项 `[!]`（MySQL/MariaDB 实机、真实 S3/MinIO/R2）复查日期 2026-09-07 |

| W16 (M16) | 已完成 | 2026-08-08 | `d953004` + `5c33551` | M16 收口：测试/安全/故障/经济/性能/发布验收 48 任务全部终态（730/783）；193 契约全绿（Problem.code 四方一致 106 码）；S3 mock 故障注入 + step-injection 回滚；实测 perf（100k 用户/1M 帖子/1137MB；p95 17-24ms；RSS 35MB）；2 项外部阻塞（MySQL/MariaDB、真实 S3） |
| W17 (M17) | 已完成 | 2026-08-08 | `c109c31` | M17 收口：RC 冻结/预发布/冒烟/Flag 记录/法律/上线 44 任务全部终态；783/783（759 完成 + 24 阻塞）；checker 接受终态标记；coverage 193/193 verified（补齐 profile-cover 与 posts 反应删除端点 + 0058 迁移） |
