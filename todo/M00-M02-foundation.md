...
- [x] `M02-IDENTITY-01..12` `[450m]` M02-IDENTITY 工作包全部完成：注册/验证/重发/找回密码全链路（事务原子性、统一响应防枚举、双维度限流、token 只存 hash、冷静期、审计/领域事件、故障注入无半完成状态）。证据：files=backend/src/auth/{identity,password,registration,verification,resend,password_reset}.rs,backend/src/ratelimit.rs,backend/src/domain/registration.rs,backend/tests/{identity_schema,identity_normalization,registration_tx,verification,resend_verification,password_reset,token_hygiene,identity_faults}.rs；commands=cargo test --all-features（333 通过/0 失败）; cargo clippy 0 warning; make check（173/173）; contract=OpenAPI 新增 resendVerification（172→173）+ 冷静期配置 + 429 rate_limited 契约；commits=4ef6fed,e379edc,79187da,6fc780f,11208d6,4bab020,319823b,60e4b55,5a70734,8c65109,95ca3fe,6f9f497；review=12 个叶子任务逐项证据 + 工作包验收
