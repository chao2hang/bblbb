...
- [x] `M11-CONSENT-11` `[30m]` 更新 OAuth/OAuth Clients operation coverage 和 Feature Flag 审批记录。 证据：files=todo/openapi-operation-coverage.json;backend/src/config/flags.rs;backend/tests/http.rs；commands=ruby scripts/sync-operation-coverage.rb --check(193/193);ruby scripts/check-roadmap.rb(exit 0)；contract=openapi-operation-coverage.json OAuth+OAuth Clients 14 ops verified+Feature Flag 变更审计；commit=<hash>；review=sync-operation-coverage.rb --check 通过
