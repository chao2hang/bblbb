...
(function () {
  ...
  var GENERIC_ADMIN = {
    ...
    '#admin-posts': ['帖子与文章', '审核状态、作者、板块与内容操作。', ['使用 SvelteKit 构建博客与轻量论坛是否合理？|待审核', 'Rust 小机器上的 SQLite 并发实践|精华', 'OAuth Provider 应该自研到什么程度？|公开', 'Markdown 渲染沙箱与 CSP 实战|已发布']],
  ...
  function pushLedger(kind, amount, source) {
    if (!Array.isArray(state.ledger)) state.ledger = [];
    state.ledger.unshift({ kind: kind, amount: amount, source: source || '原型演示', time: new Date().toISOString() });
  ...
  /* 统一页面标题区：kicker / 标题 / 导语 / 右侧操作按钮。 */
  function routeIntro(kicker, title, lede, actions) {
    var html = '<div class="app-page-head"><div class="app-page-head__copy">';
    if (kicker) html += '<span class="app-kicker">' + esc(kicker) + '</span>';
    html += '<h1>' + esc(title || '') + '</h1>';
    if (lede) html += '<p>' + esc(lede) + '</p>';
    html += '</div>';
    if (actions) html += '<div class="app-page-head__actions">' + actions + '</div>';
    html += '</div>';
    return html;
  }
  function topicContext(post) {
    var kind = post.type === 'article' ? '文章详情' : '讨论详情';
    return '<nav class="topic-context" aria-label="讨论位置"><a href="#home">首页</a><span aria-hidden="true">/</span><a href="#board:' + encodeURIComponent(post.board) + '">' + esc(post.boardName) + '</a><span aria-hidden="true">/</span><span class="topic-context__current">' + kind + '</span></nav>';
  }
  ...
  function bindHomeInteractions() {
    ...
    qa('.filters button', page).forEach(function (b, i) { b.onclick = function () { qa('.filters button', page).forEach(function (x) { x.classList.remove('active'); }); b.classList.add('active'); if (i === 3) { var menu = q('[data-home-type-menu]', page); if (menu) menu.hidden = !menu.hidden; } else if (i < 3) { var kind = i === 0 ? 'all' : i === 1 ? 'featured' : 'followed'; qa('#thread-list .thread', page).forEach(function (row) { row.hidden = kind === 'featured' ? !row.classList.contains('featured') : kind === 'followed' ? row.getAttribute('data-followed') !== 'true' : false; }); } }; });
    var typeButton = qa('.filters button', page)[3];
    if (typeButton && !q('[data-home-type-menu]', page)) { var menu = document.createElement('div'); menu.setAttribute('data-home-type-menu', 'true'); menu.className = 'home-type-menu'; menu.hidden = true; menu.innerHTML = '<button type="button" data-home-type="article">文章</button><button type="button" data-home-type="topic">讨论</button><button type="button" data-home-type="all">全部类型</button>'; typeButton.parentElement.appendChild(menu); qa('button', menu).forEach(function (m) { m.onclick = function () { var kind = m.getAttribute('data-home-type'); qa('#thread-list .thread', page).forEach(function (row) { row.hidden = kind === 'all' ? false : row.getAttribute('data-post-type') !== kind; }); menu.hidden = true; toast('已筛选：' + m.textContent); }; }); }
  ...
  function renderBoard(slug) {
    ...
    mountTpl('board', 'board', function () {
      var boardKicker = q('.app-page-head .app-kicker'); if (boardKicker) boardKicker.textContent = 'COMMUNITY / ' + board.slug.toUpperCase();
      var boardH1 = q('.app-page-head h1'); if (boardH1) boardH1.textContent = board.name;
      var boardLede = q('.app-page-head p'); if (boardLede) boardLede.textContent = board.desc;
  ...
  function renderTags(tag) {
    if (tag) {
      ...
      mountTpl('tag', 'tag', function () {
        var tagH1 = q('.app-page-head h1'); if (tagH1) tagH1.textContent = '标签：' + tag;
        var tagLede = q('.app-page-head p'); if (tagLede) tagLede.textContent = '聚合所有包含该标签的文章与讨论';
  ...
  function renderTopic(id) {
    ...
    mountTpl('topic', 'topic', function () {
      ...
      var crumbNow = q('.topic-context__current', topicPage); if (crumbNow) crumbNow.textContent = post.type === 'article' ? '文章详情' : '讨论详情';
      var metaSpans = qa('.topic-meta > span', topicPage);
      if (metaSpans[0]) metaSpans[0].textContent = post.type === 'article' ? '文章' : '讨论';
  ...
  function renderPublish(mode) {
    var isTopic = String(mode || '').indexOf('topic') === 0;
    ...
    mountTpl('publish', 'publish', function () {
      var pubIsTopic = String(mode || '').indexOf('topic') === 0;
      qa('[data-publish-type]').forEach(function (b) { b.classList.toggle('is-active', b.getAttribute('data-publish-type') === (pubIsTopic ? 'topic' : 'article')); });
      var pubH1 = q('#page-publish h1'); if (pubH1) pubH1.textContent = pubIsTopic ? '发布讨论' : '发布文章';
      var pubEyebrow = q('.composer-eyebrow'); if (pubEyebrow) pubEyebrow.textContent = 'CREATE / ' + (pubIsTopic ? 'TOPIC' : 'ARTICLE');
      var pubContext = q('.topic-context__current'); if (pubContext) pubContext.textContent = pubIsTopic ? '发布讨论' : '发布文章';
  ...
  function wirePublish(modeIsTopic) {
    ...
    qa('[data-publish-type]').forEach(function (btn) {
      btn.addEventListener('click', function () {
        var type = btn.getAttribute('data-publish-type');
        go('#publish:' + type);
      });
    });
      ...
      q('[data-submit-review]').addEventListener('click', function () {
        ...
        if (confirmBtn) {
          confirmBtn.addEventListener('click', function () {
            var reviewPost = Object.assign({ id: 'draft-' + Date.now(), status: 'pending_review', author: (state.account && state.account.displayName) || 'Chaos', type: modeIsTopic ? 'topic' : 'article', boardName: ((BOARDS.find(function (b) { return b.slug === pp.board; }) || {}).name) || '板块', views: '1' }, publishPayload());
    ...
    if (q('[data-submit-publish]')) {
      q('[data-submit-publish]').addEventListener('click', function () {
        ...
        if (confirmBtn) {
          confirmBtn.addEventListener('click', function () {
            var newPost = Object.assign({ id: 'post-' + Date.now(), status: payload.schedule === 'scheduled' ? 'scheduled' : 'published', author: (state.account && state.account.displayName) || 'Chaos', type: modeIsTopic ? 'topic' : 'article', boardName: ((BOARDS.find(function (b) { return b.slug === payload.board; }) || {}).name) || '板块', views: '1' }, payload);
  ...
  function renderUser(name, target) {
    ...
    var tabs = [['posts', '文章'], ['topics', '讨论'], ['replies', '回复'], ['activity', '动态']];
    ...
    list += published;
    var topicRows = Object.keys(TOPICS).map(function (k) { return TOPICS[k]; }).filter(function (p) { return p.author === user.name; }).map(function (p) { return '<a class="app-post-row" href="#topic:' + p.id + '">' + profileBadge(user.name, user.initial) + '<span class="app-post-row__main"><h3>' + esc(p.title) + '</h3><p>' + esc(p.boardName) + ' · ' + esc(p.views) + ' 浏览</p></span></a>'; }).join('');
    ...
    var activityRows = [['发布了一篇文章', '「' + (ARTICLES[0].title) + '」', '昨天'], ['解锁了成就「签到达人」', '连续签到 30 天', '3 天前'], ['发布了新讨论', '「' + (TOPICS['201'] ? TOPICS['201'].title : '新讨论') + '」', '1 周前']].map(function (x) { return '<div class="app-post-row"><span class="app-post-row__main"><h3>' + x[0] + '</h3><p>' + x[1] + '</p></span><span class="app-post-row__right"><span class="app-muted">' + x[2] + '</span></span></div>'; }).join('');
    var tabExtra = { topics: topicRows || empty('inbox', '还没有讨论', '去板块里发起第一个话题', button('去发现', 'primary', 'data-go-discover')), replies: replyRows, activity: activityRows };
    ...
    mountTpl((target || 'user'), (target === 'me' ? 'me' : 'user'), function () {
      var userH1 = q('.app-page-head h1'); if (userH1) userH1.textContent = own ? '我的' : user.name;
      var userActionsHost = q('.app-page-head__actions'); if (userActionsHost) userActionsHost.innerHTML = actions;
  ...
  function renderFavorites() {
    ...
    mountTpl('favorites', 'favorites', function () {
      var favLede = q('.app-page-head p'); if (favLede) favLede.textContent = '收藏的文章与讨论 · ' + rows.length + ' 项';
  ...
  function renderMessages() {
    ...
    mountTpl('messages', 'messages', function () {
      ...
      var msgBody = q('[data-chat-body]'); if (msgBody) msgBody.innerHTML = renderConversationBody(activeName);
      var msgFailBtn = q('[data-message-fail]'); if (msgFailBtn) msgFailBtn.textContent = state.messageFail ? '关闭发送失败模拟' : '模拟发送失败';

    qa('[data-conversation]').forEach(function (b) { b.addEventListener('click', function () { var name = b.getAttribute('data-conversation'); state.activeConversation = name; saveState(); renderMessages(); }); });
  ...
  function renderSearch(initialQuery) {
    ...
    mountTpl('search', 'search', function () {
      var searchLede = q('.app-page-head p'); if (searchLede) searchLede.textContent = query ? '正在搜索：' + query : '搜索帖子、用户或标签';
 ...
 qa('[data-market-offer]').forEach(function (b) { b.addEventListener('click', function () { openModal('市场接入', '<p>创建 checkout intent，确认 scope、单笔限额和授权有效期。</p>', '取消 ' + button('继续', 'primary', 'data-market-confirm')); q('[data-market-confirm]').addEventListener('click', function () { closeModal(); go('#checkout'); }); }); }); qa('[data-market-purchase]').forEach(function (b) { b.addEventListener('click', function () { toast('交易详情：原子事务已提交，等待 Webhook 对账'); }); });
  ...
  function adminSide(active) {
    return '<div class="app-admin-side__brand">BBLBB Admin</div>' + ADMIN_GROUPS.map(function (group) { return '<div class="app-admin-side__group"><span class="app-admin-side__label">' + group[0] + '</span>' + group[1].map(function (href) { var item = ADMIN_ITEMS.find(function (x) { return x[0] === href; }); return '<a class="' + (href === active ? 'is-active' : '') + '" href="' + href + '">' + icon(item ? item[2] : 'grid') + (item ? item[1] : href) + '</a>'; }).join('') + '</div>'; }).join('') + '<div class="app-admin-side__foot">' + (state.role === 'admin' ? '管理员演示态' : '普通成员演示态') + ' · 所有变更均需真实 API、权限和审计确认</div>';
  }
  function adminShell(active, title, lede, content, actions) {
    var kicker = 'ADMIN' + (active !== '#admin' ? ' / ' + active.slice('#admin-'.length).toUpperCase() : '');
    mount('admin', '<div class="app-admin-shell"><aside class="app-admin-side">' + adminSide(active) + '</aside><div class="app-admin-main">' + routeIntro(kicker, title, lede, actions || '') + content + '</div></div>');
  }
  ...
  function renderReports(route) {
    if (route !== '#admin-reports') {
      ...
      mountTpl('admin', 'admin-report', function () {
        ...
        if (reportMain) reportMain.innerHTML = routeIntro('ADMIN / REPORT', '举报 ' + id, meta.title + ' · 被举报人 ' + meta.accused) + detailContent;
    ...
    mountTpl('admin', 'admin-reports', function () {
      var reportsLede = q('.app-page-head p'); if (reportsLede) reportsLede.textContent = '待处理 ' + counts.pending + ' · 处理中 ' + counts.processing + ' · 历史 18';
  ...
  function applyReportFilter(key) {
    var visible = 0;
    qa('[data-report-row]').forEach(function (row) { var st = row.getAttribute('data-report-row'); var ok = key === 'all' || st === key; row.hidden = !ok; if (ok) visible += 1; });
  ...
  function renderPoints() {
    mountTpl('admin', 'admin-points', function () {
      pointsSyncResult();
      q('[data-points-query]').addEventListener('click', function () { var name = q('[data-points-user]').value.trim(); q('[data-points-result]').innerHTML = name.toLowerCase() === 'chaos' ? '<div class="app-profile" style="margin-top:12px">' + profileBadge('Chaos', 'C') + '<div><h2>Chaos</h2><p>经验 ' + state.balances.exp + ' · B币 ' + state.balances.coin + ' · 贡献 ' + state.balances.contrib + '</p></div>' + button('调整积分', 'primary', 'data-adjust-points') + '</div>' : empty('search', '未找到用户', '请检查用户名后重试'); wirePointsAdjust(); });
      wirePointsAdjust();
    });
  }
  function pointsSyncResult() {
    var host = q('[data-points-result]');
    if (host) host.innerHTML = '<div class="app-profile" style="margin-top:12px">' + profileBadge('Chaos', 'C') + '<div><h2>Chaos</h2><p>经验 ' + state.balances.exp + ' · B币 ' + state.balances.coin + ' · 贡献 ' + state.balances.contrib + '</p></div>' + button('调整积分', 'primary', 'data-adjust-points') + '</div>';
  }
  ...
  function wirePointsAdjust() {
    ...
    adjust.__wired = true;
    adjust.addEventListener('click', function () { openModal('调整积分', '<label class="app-field-label">币种</label><select class="app-select" data-adjust-kind><option value="coin">B币</option><option value="exp">经验</option><option value="contrib">贡献</option></select><label class="app-field-label" style="margin-top:12px">增加数额</label><input class="app-field" aria-label="调整数额" type="number" value="50" data-adjust-amount><label class="app-field-label" style="margin-top:12px">原因 <span class="app-required">*</span></label><textarea class="app-textarea" aria-label="调整原因" data-adjust-reason placeholder="必填，流水不可删除"></textarea>', '取消 ' + button('下一步', 'primary', 'data-confirm-adjust')); q('[data-confirm-adjust]').addEventListener('click', function () { var reason = q('[data-adjust-reason]').value.trim(); if (!reason) { toast('请填写原因'); return; } var amount = Number(q('[data-adjust-amount]').value) || 0; if (amount <= 0) { toast('数额必须大于 0'); return; } openModal('二次确认', '<p>历史流水不可删除，仅可创建补偿记录。</p><p>确认增加 ' + amount + '？</p>', '取消 ' + button('确认调整', 'primary', 'data-final-adjust')); q('[data-final-adjust]').addEventListener('click', function () { var kind = q('[data-adjust-kind]').value; state.balances[kind] += amount; saveState(); closeModal(); renderPoints(); toast('调整已生效 · 流水已追加'); }); }); });
  }
  function renderLevels() {
    var LEVELS = [
      { code: 'Lv0', name: 'Lv.0', exp: '0', perks: '2 MB 附件' },
      { code: 'Lv1', name: 'Lv.1', exp: '300', perks: '5 MB 附件' },
      { code: 'Lv2', name: 'Lv.2', exp: '1000', perks: '10 MB 附件 · 可 @ 官方账号' },
      { code: 'Lv3', name: 'Lv.3', exp: '2000', perks: '20 MB 附件 · 版主申请' },
      { code: 'Lv4', name: 'Lv.4', exp: '5000', perks: '50 MB 附件' }
    ];
    mountTpl('admin', 'admin-levels', function () {

    qa('[data-level-tab]').forEach(function (tab) {
      tab.addEventListener('click', function () {
        qa('[data-level-tab]').forEach(function (x) { x.classList.remove('is-active'); x.setAttribute('aria-selected', 'false'); });
        tab.classList.add('is-active'); tab.setAttribute('aria-selected', 'true');
        var code = tab.getAttribute('data-level-tab');
        qa('[data-level-row]').forEach(function (row) { row.classList.toggle('is-highlighted', row.getAttribute('data-level-row') === code); });
      });
    });
    qa('[data-level-edit]').forEach(function (b) { b.addEventListener('click', function () { openModal('编辑等级', '<label class="app-field-label">名称</label><input class="app-field" value="Lv.3"><label class="app-field-label">经验门槛</label><input class="app-field" value="2000"><label class="app-field-label">权益</label><textarea class="app-textarea">20 MB 附件</textarea>', '取消 ' + button('保存', 'primary', 'data-save-level')); q('[data-save-level]').addEventListener('click', function () { closeModal(); toast('等级配置已保存'); }); }); });
    });
}
  function renderAchievementsAdmin() { mountTpl('admin', 'admin-achievements', function () {
 q('[data-create-achievement]').addEventListener('click', function () { openModal('新建成就', '<label class="app-field-label">code *</label><input class="app-field" placeholder="unique_code"><label class="app-field-label">标题</label><input class="app-field"><label class="app-field-label">条件</label><textarea class="app-textarea"></textarea>', '取消 ' + button('创建', 'primary', 'data-save-ach')); q('[data-save-ach]').addEventListener('click', function () { closeModal(); toast('成就已创建'); }); }); qa('[data-ach-more]').forEach(function (b) { b.addEventListener('click', function () { openModal('成就操作', '<p>可编辑、停用、手工授予、撤销用户成就或归档。</p><p class="app-muted">删除按钮不存在；有人解锁过的成就不可归档。</p>', '关闭'); }); });   });
}
 ...
 q('[data-create-oauth]').addEventListener('click', function () { openModal('创建 OAuth 客户端', '<label class="app-field-label">应用名称</label><input class="app-field" aria-label="应用名称" data-oauth-name placeholder="Demo App"><label class="app-field-label">类型</label><select class="app-select"><option>Confidential</option><option>Public</option></select><label class="app-field-label">Redirect URI</label><input class="app-field" aria-label="Redirect URI" placeholder="https://example.com/callback"><label class="app-field-label">Scopes</label><div class="app-radio-list"><label><input type="checkbox" checked> openid</label><label><input type="checkbox" checked> profile</label><label><input type="checkbox"> email</label></div>', '取消 ' + button('创建', 'primary', 'data-create-oauth-confirm')); q('[data-create-oauth-confirm]').addEventListener('click', function () { var name = q('[data-oauth-name]').value.trim() || 'Demo App'; closeModal(); openModal('凭据仅显示一次', '<p>请立即保存 Client Secret，关闭后不可再次查看。</p><div class="app-secret"><input readonly value="cli_demo_' + Date.now() + '">' + button('复制', 'ghost', 'data-copy-share') + '</div><div class="app-secret"><input readonly value="cs_demo_secret_once">' + button('复制', 'ghost', 'data-copy-share') + '</div>', '我已保存'); toast('客户端 ' + name + ' 已创建'); }); }); qa('[data-oauth-action]').forEach(function (b) { b.addEventListener('click', function () { openModal('确认操作', '<p>该操作会写入审计日志。</p>', '取消 ' + button('确认', 'danger', 'data-confirm-oauth-action')); q('[data-confirm-oauth-action]').addEventListener('click', function () { closeModal(); b.textContent = '已完成'; b.disabled = true; toast('OAuth 状态已更新'); }); }); });   });
  ...
  var ADM_DOWNLOADS = [
    { id: 'DL-8823', file: 'report.png', user: 'Yuwen', size: '1.2 MB', status: '排队中' },
    { id: 'DL-8822', file: 'architecture.pdf', user: 'Alice', size: '2.4 MB', status: '生成中' },
  ...
  var ADM_APPS = [
    { id: 'theme-store', name: '主题商店', kind: '应用', status: '已启用', orders: 42 },
    { id: 'avatar-gen', name: '头像生成', kind: '工具', status: '已启用', orders: 128 },
    { id: 'resume-tpl', name: '简历模板', kind: '内容', status: '待审批', orders: 0 },
    { id: 'export-helper', name: '导出助手', kind: '工具', status: '已禁用', orders: 77 }
  ];
  var ADM_BI = {
  ...
  var ADM_ADMIN_USERS = [
    { name: 'Chaos', level: 'LV.3', coin: 328, contrib: 146, status: '正常', active: '今天', title: 'Rust / 自托管爱好者', bio: '写 Rust 和自托管，维护这个论坛。' },
    { name: 'Yuwen', level: 'LV.4', coin: 1200, contrib: 312, status: '已验证', active: '今天', title: '站长与产品维护者', bio: '把社区做成一个可以长期维护的地方。' },
    { name: 'Mark', level: 'LV.2', coin: 124, contrib: 58, status: '正常', active: '昨天', title: '全栈开发者', bio: '记录工程实践与产品反馈。' },
    { name: 'Alice', level: 'LV.4', coin: 14, contrib: 22, status: '已验证', active: '3 天前', title: '后端与数据库调优', bio: '喜欢把复杂系统拆成可验证的小步。' },
    { name: 'Nina', level: 'LV.3', coin: 412, contrib: 88, status: '正常', active: '今天', title: '性能与可观测性', bio: '专注服务端性能优化与压测。' },
    { name: 'spam_bot_42', level: 'LV.1', coin: 0, contrib: 0, status: '封禁', active: '30 天前', title: '—', bio: '因垃圾广告被禁言。' }
  ];
  var ADM_ROLES = [
    { id: 'admin', name: '管理员', scope: '全站', perms: ['admin.manage', 'audit.read', 'user.ban', 'points.adjust', 'board.manage', 'post.manage'] },
    { id: 'mod', name: '版主', scope: '所在板块', perms: ['board.moderate', 'post.manage', 'report.handle'] },
    { id: 'member', name: '成员', scope: '个人', perms: ['content.create', 'content.comment', 'content.favorite'] },
    { id: 'guest', name: '访客', scope: '公开内容', perms: ['public.read'] }
  ];
  var ADM_POSTS = [
  ...
  function rolePermsOf(roleId) {
    var role = ADM_ROLES.filter(function (r) { return r.id === roleId; })[0];
    var saved = (state.rolePerms || {})[roleId] || {};
    var out = {};
    if (!role) return out;
    role.perms.forEach(function (p) { out[p] = saved[p] == null ? true : !!saved[p]; });
    return out;
  }
  ...
  function renderDownloadBilling() {
    ...
    var qmap = state.downloadQueue || {};
    var rows = ADM_DOWNLOADS.map(function (d) {
      var st = qmap[d.id] || d.status;
      var stCls = st === '已完成' ? 'sb-success' : st === '失败' ? 'sb-danger' : st === '已取消' ? 'sb-gray' : 'sb-hot';
      var acts = '';
      if (st === '失败') acts += button('重试', 'ghost sm', 'data-adm-act="dl-retry" data-adm-id="' + d.id + '"');
      if (st === '排队中' || st === '生成中') acts += button('取消', 'ghost sm', 'data-adm-act="dl-cancel" data-adm-id="' + d.id + '"');
    ...
    q('[data-dl-export]').addEventListener('click', function () {
      var qm = state.downloadQueue || {};
      if (adminCsv('bblbb-download-billing.csv', ['单号', '文件', '用户', '大小', '状态'], ADM_DOWNLOADS.map(function (d) { return [d.id, d.file, d.user, d.size, qm[d.id] || d.status]; }))) { logAudit('导出下载账单', ADM_DOWNLOADS.length + ' 条'); toast('账单已导出 · ' + ADM_DOWNLOADS.length + ' 条'); }
    });
    q('[data-dl-refresh]').addEventListener('click', function () {
      actionPending(this, '刷新中…', function () {
        var qm = state.downloadQueue || {};
        ADM_DOWNLOADS.forEach(function (d) { var st = qm[d.id] || d.status; if (st === '排队中' || st === '生成中') qm[d.id] = '已完成'; });
        state.downloadQueue = qm; saveState(); logAudit('刷新下载队列', '进行中任务推进为已完成'); renderDownloadBilling(); toast('队列已刷新 · 进行中任务已完成');
      });
    });
    ...
    bindAdminActs(function (el, act, id) {
      ...
      if (act === 'dl-detail') { openModal('下载 ' + id, '<p><b>' + d.file + '</b> · ' + d.size + '</p><p class="app-muted">用户 ' + d.user + ' · 当前状态 ' + ((state.downloadQueue || {})[id] || d.status) + ' · 授权有效期 ' + (state.billingConfig || { ttl: '24 小时' }).ttl + '</p>', '关闭'); return; }
      if (act === 'dl-retry') { actionPending(el, '重新入队…', function () { var qm = state.downloadQueue || {}; qm[id] = '生成中'; state.downloadQueue = qm; saveState(); logAudit('重试下载 ' + id, d.file); renderDownloadBilling(); toast(id + ' 已重新入队'); }); return; }
      if (act === 'dl-cancel') confirmAdmin('取消下载', '取消 ' + id + '（' + d.file + '）？用户需要重新申请。', '取消下载', function () { var qm = state.downloadQueue || {}; qm[id] = '已取消'; state.downloadQueue = qm; saveState(); logAudit('取消下载 ' + id, d.file); renderDownloadBilling(); toast('下载已取消 · 已写入审计'); });
    });
    });
}
  ...
  function renderAiAdmin() {
    ...
    mountTpl('admin', 'admin-ai', function () {
      ...
      var aiProvider = q('[data-ai-provider]'); if (aiProvider) aiProvider.value = cfg.provider;
      var aiModel = q('[data-ai-model]'); if (aiModel) aiModel.value = cfg.model;
    ...
    q('[data-ai-test]').addEventListener('click', function () { actionPending(this, '测试中…', function () { toast('连接正常 · 延迟 86ms · 模型 ' + q('[data-ai-model]').value); logAudit('大模型连接测试', q('[data-ai-model]').value); }); });
    q('[data-ai-save]').addEventListener('click', function () {
      state.aiConfig = { provider: q('[data-ai-provider]').value, model: q('[data-ai-model]').value.trim() || 'bblbb-format-v1', privacy: q('[data-ai-privacy]').value, budget: Math.max(0, Number(q('[data-ai-budget]').value) || 0), rpd: Math.max(0, Number(q('[data-ai-rpd]').value) || 0) };
  ...
  function renderMarketplaceAdmin() {
    ...
    var rows = ADM_APPS.map(function (a) {
      ...
      acts += button('查看', 'ghost sm', 'data-adm-act="mp-view" data-adm-id="' + a.id + '"');
    ...
    bindAdminActs(function (el, act, id) {
      ...
      if (act === 'mp-view') { openModal(a.name, '<p>' + a.kind + ' · 累计订单 ' + a.orders + '</p><p class="app-muted">当前状态 ' + ((state.appsStatus || {})[id] || a.status) + ' · 结算 T+1 · Webhook 通知已启用 · 沙箱验证通过</p>', '关闭'); return; }
  ...
  function renderBiAdmin() {
    ...
    q('[data-bi-refresh]').addEventListener('click', function () {
    ...
    q('[data-bi-export]').addEventListener('click', function () {
  ...
  function usersBody() {
    ...
    var list = ADM_ADMIN_USERS.filter(function (u) { return !term || u.name.toLowerCase().indexOf(term) >= 0; });
    ...
    return list.map(function (u) {
      ...
      return '<tr data-user-row="' + esc(u.name) + '"><td><div class="app-user-cell">' + profileBadge(u.name, u.name.charAt(0)) + '<span><b>' + esc(u.name) + '</b>' + (u.name === 'Chaos' ? ' <span class="sbadge sb-brand">你</span>' : '') + '<small class="sub">' + esc(u.title) + '</small></span></div></td><td>' + u.level + '</td><td class="num">' + u.coin + '</td><td class="num">' + u.contrib + '</td><td><span class="sbadge ' + stCls + '">' + st + '</span></td><td>' + u.active + '</td><td class="adm-acts">' + acts + '</td></tr>';
    }).join('');
  }
  function renderUsersAdmin() {
    
    mountTpl('admin', 'admin-users', function () {
      var usersHost = q('[data-users-body]'); if (usersHost) usersHost.innerHTML = adminTable(['用户', '等级', 'B币', '贡献', '状态', '最近活动', '操作'], usersBody());

    q('[data-users-q]').addEventListener('input', function () { var b = q('[data-users-body]'); if (b) b.innerHTML = usersBody(); });
    ...
    bindAdminActs(function (el, act, id) {
      ...
      if (act === 'us-view') {
        openModal(u.name + ' · 用户详情', '<p>' + profileBadge(u.name, u.name.charAt(0)) + '</p><p class="app-muted">' + u.level + ' · ' + esc(u.title) + '</p><p>' + esc(u.bio) + '</p><p class="app-muted">B币 ' + u.coin + ' · 贡献 ' + u.contrib + ' · 最近活动 ' + u.active + '</p><p>状态：<span class="sbadge ' + (userStatusOf(u.name) === '封禁' ? 'sb-danger' : 'sb-success') + '">' + userStatusOf(u.name) + '</span></p><p class="app-muted">积分流水：' + ((state.userLedger || {})[u.name] || []).length + ' 条调整记录</p>', '关闭 ' + button('查看主页', 'ghost', 'data-us-goto'));
        q('[data-us-goto]').addEventListener('click', function () { closeModal(); go('#user:' + encodeURIComponent(u.name)); });
        return;
      }
  ...
  function renderRolesAdmin() {
    ...
    mountTpl('admin', 'admin-roles', function () {
      qa('[data-role-perm]').forEach(function (cb) { var parts = cb.getAttribute('data-role-perm').split(':'); cb.checked = rolePermsOf(parts[0])[parts[1]] === true; });

    bindAdminActs(function (el, act, id) {
      var r = ADM_ROLES.filter(function (x) { return x.id === id; })[0]; if (!r) return;
      if (act === 'rl-preview') {
        var m = rolePermsOf(id);
        var list = r.perms.filter(function (p) { return m[p]; });
        openModal(r.name + ' · 生效权限', '<ul class="app-perm-list">' + (list.length ? list.map(function (p) { return '<li><code>' + p + '</code></li>'; }).join('') : '<li class="app-muted">无生效权限</li>') + '</ul><p class="app-muted">' + list.length + ' / ' + r.perms.length + ' 项权限开启 · 范围：' + r.scope + '</p>', '关闭');
        return;
      }
      if (act === 'rl-reset') { var m2 = state.rolePerms || {}; var def = {}; r.perms.forEach(function (p) { def[p] = true; }); m2[id] = def; state.rolePerms = m2; saveState(); logAudit('恢复角色默认权限', r.name); renderRolesAdmin(); toast(r.name + ' 权限已恢复默认'); return; }
      if (act === 'rl-save') {
        var m3 = state.rolePerms || {}; var next = {};
        qa('[data-role-perm]').forEach(function (cb) { var key = cb.getAttribute('data-role-perm'); if (key.indexOf(id + ':') === 0) next[key.slice(id.length + 1)] = cb.checked; });
        m3[id] = next; state.rolePerms = m3; saveState();
        var off = r.perms.filter(function (p) { return !next[p]; });
  ...
  function postsBody(key) {
    ...
    if (!list.length) return '<tr><td colspan="6">' + empty('file-text', '该状态下没有帖子', '切换其他状态查看。') + '</td></tr>';
    return list.map(function (p) {
      ...
      return '<tr><td><b>' + esc(p.title) + '</b><span class="sub">' + sub + '</span></td><td>' + p.author + '</td><td>' + p.board + '</td><td><span class="sbadge ' + stCls + '">' + st + '</span></td><td>' + p.time + '</td><td class="adm-acts">' + acts + '</td></tr>';
    }).join('');
  }
  function renderPostsAdmin() {
    ...
    mountTpl('admin', 'admin-posts', function () {
      ...
      bindAdminActs(function (el, act, id) {
        ...
        if (act === 'ps-reject') { confirmAdmin('驳回帖子', '驳回「' + esc(p.title) + '」？作者会收到驳回通知。', '驳回并隐藏', function () { set('已隐藏', '审核驳回'); }); return; }
  ...
  function renderAttachmentsAdmin() {
    ...
    var rows = ADM_FILES.map(function (f) {
      ...
      if (del) acts = '<span class="app-muted">已移除</span>';
        ...
        if (st !== '已通过') acts += button('扫描', 'ghost sm', 'data-adm-act="fl-scan" data-adm-id="' + f.id + '"');
        acts += button('临时链接', 'ghost sm', 'data-adm-act="fl-link" data-adm-id="' + f.id + '"') + button('删除', 'danger sm', 'data-adm-act="fl-del" data-adm-id="' + f.id + '"');
      }
      return '<tr style="' + (del ? 'opacity:.55' : '') + '"><td><b>' + esc(f.name) + '</b><span class="sub">' + f.id + '</span></td><td>' + f.user + '</td><td class="num">' + f.size + '</td><td>' + f.scope + '</td><td>' + (del ? '<span class="sbadge sb-gray">已删除</span>' : '<span class="sbadge ' + stCls + '">' + st + '</span>') + '</td><td class="adm-acts">' + acts + '</td></tr>';
    }).join('');
    ...
    mountTpl('admin', 'admin-settings', function () {
      ...
      q('[data-sys-reset]').addEventListener('click', function () {
        confirmAdmin('重置演示数据', '将清除 localStorage 中的全部演示状态（登录、积分、草稿、后台变更）并刷新页面。', '确认重置', function () {
          try { localStorage.removeItem(APP_KEY); localStorage.removeItem('returnTo'); sessionStorage.removeItem('returnTo'); } catch (e) {}
          location.reload();
        });
      });
      refresh();
  ...
  function renderAdmin(route) {
    ...
    else if (route === '#admin-users') renderUsersAdmin();
  ...
  function renderRoute(raw) {
    ...
    if (base === 'articles') { hidePages(); renderArticles(); setDocumentTitle('文章'); return; }
