...
async fn register_endpoint_returns_unified_response() {
    ...
    let first = register("alice", "alice@example.com").await.unwrap();
    assert_eq!(first.status(), StatusCode::CREATED);
    let first_body = first.into_body().collect().await.unwrap().to_bytes();

    let second = register("ALICE", "other@example.com").await.unwrap();
    ...
    let err_body = register("bob", "not-an-email").await.unwrap();
    assert_eq!(
        err_body.status(),
